...
# parent directory of subject folders
path=/storage/users/yyi/coregistration

# MNI
MNI=/storage/users/yyi/coregistration/mni_icbm152_t1_tal_nlin_asym_09c.nii

# template
template=/storage/users/yyi/coregistration/hc_template.nii.gz

# assign number of CPUs used
CPUs=3

# subject folders in ${path}
ls -d "${path}"/*/ > "${path}"/participants_MRPET.txt
...
while read folder; do
	...
	mri_convert -cs 1 -odt float -rl "${folder}"data/T1WB.nii -rt cubic "${folder}"data/LCmask_"${ID}".nii.gz "${folder}"data/LCmask_"${ID}"_mask_CONJ_1mm_backup.nii
...
done < "${path}"/participants_MRPET.txt
