...
paths.analyses= [paths.parent 'E_data/EB_cleaned/EBD_mrpet/RewardTask/MRI/bothSessions/1st/'];
...
for id = 1:length(IDs)
    ...
    for d = 1:2
        if days(id,d) == 0
            ...
            % sort onsets
            onsets_temp{id,d}.stim.reward  = tmponsets.stim(ind_rew);
            onsets_temp{id,d}.stim.neutral = tmponsets.stim(ind_neu);
            onsets_temp{id,d}.null         = tmponsets.null;
            onsets_temp{id,d}.resp         = tmponsets.resp;
            onsets_temp{id,d}.fb.reward    = tmponsets.cue(ind_rew);
            onsets_temp{id,d}.fb.neutral   = tmponsets.cue(ind_neu);
...
% %% make physio+realignment+session regressors (done)
% 
% for id = 1:length(IDs)
...
path_destination ='/Users/yeojin/Desktop/E_data/ED_coreg/mrpet_both/';
...
paths.save2nd = [paths.parent 'E_data/EB_cleaned/EBD_mrpet/RewardTask/MRI/bothSessions/2ndLvL/'];
...
list_stim_highDA_reward_vs_neutral = [];
for c1 = 1:length(IDs)
    list_stim_highDA_reward_vs_neutral{c1,1} = [paths.analyses num2str(IDs(c1)) '_' num2str(days(c1,d)) '/con_0001_mni.nii,1'];
end

list_stim_lowDA_reward_vs_neutral = []; 
for c1 = 1:length(IDs)
            list_stim_lowDA_reward_vs_neutral{c1,1} = [paths.analyses num2str(IDs(c1)) '_' num2str(days(c1,d)) '/con_0002_mni.nii,1'];
end

list_stim_rewardAll_vs_neutralAll = [];
for c1 = 1:length(IDs)
            list_stim_rewardAll_vs_neutralAll{c1,1} = [paths.analyses num2str(IDs(c1)) '_' num2str(days(c1,d)) '/con_0003_mni.nii,1'];
end

list_fb_highDA_reward_vs_neutral = [];
for c1 = 1:length(IDs)
            list_fb_highDA_reward_vs_neutral{c1,1} = [paths.analyses num2str(IDs(c1)) '_' num2str(days(c1,d)) '/con_0004_mni.nii,1'];
end

list_fb_lowDA_reward_vs_neutral = [];
for c1 = 1:length(IDs)
    list_fb_lowDA_reward_vs_neutral{c1,1} = [paths.analyses num2str(IDs(c1)) '_' num2str(days(c1,d)) '/con_0005_mni.nii,1'];
end

list_fb_rewardAll_vs_neutralAll = [];
for c1 = 1:length(IDs)
    list_fb_rewardAll_vs_neutralAll{c1,1} = [paths.analyses num2str(IDs(c1)) '_' num2str(days(c1,d)) '/con_0006_mni.nii,1'];
end

list_stim_Rewards_highDA_vs_lowDA = [];
for c1 = 1:length(IDs)
    list_stim_Rewards_highDA_vs_lowDA{c1,1} = [paths.analyses num2str(IDs(c1)) '_' num2str(days(c1,d)) '/con_0007_mni.nii,1'];
end

list_fb_Rewards_highDA_vs_lowDA = [];
for c1 = 1:length(IDs)
    list_fb_Rewards_highDA_vs_lowDA{c1,1} = [paths.analyses num2str(IDs(c1)) '_' num2str(days(c1,d)) '/con_0008_mni.nii,1'];
end

list_stim_Neutrals_highDA_vs_lowDA = [];
for c1 = 1:length(IDs)
    list_stim_Neutrals_highDA_vs_lowDA{c1,1} = [paths.analyses num2str(IDs(c1)) '_' num2str(days(c1,d)) '/con_0009_mni.nii,1'];
end

list_fb_Neutrals_highDA_vs_lowDA = [];
for c1 = 1:length(IDs)
    list_fb_Neutrals_highDA_vs_lowDA{c1,1} = [paths.analyses num2str(IDs(c1)) '_' num2str(days(c1,d)) '/con_0010_mni.nii,1'];
end

list_stim_highDA_vs_lowDA = [];
for c1 = 1:length(IDs)
    list_stim_highDA_vs_lowDA{c1,1} = [paths.analyses num2str(IDs(c1)) '_' num2str(days(c1,d)) '/con_0011_mni.nii,1'];
end

list_fb_highDA_vs_lowDA = [];
for c1 = 1:length(IDs)
    list_fb_highDA_vs_lowDA{c1,1} = [paths.analyses num2str(IDs(c1)) '_' num2str(days(c1,d)) '/con_0012_mni.nii,1'];
end

list_Stimuli_vs_Feedbacks = [];
for c1 = 1:length(IDs)
    list_Stimuli_vs_Feedbacks{c1,1} = [paths.analyses num2str(IDs(c1)) '_' num2str(days(c1,d)) '/con_0013_mni.nii,1'];
end

list_Feedbacks_vs_Stimuli = [];
for c1 = 1:length(IDs)
    list_Feedbacks_vs_Stimuli{c1,1} = [paths.analyses num2str(IDs(c1)) '_' num2str(days(c1,d)) '/con_0014_mni.nii,1'];
end

list_Nulls = [];
for c1 = 1:length(IDs)
    list_Nulls{c1,1} = [paths.analyses num2str(IDs(c1)) '_' num2str(days(c1,d)) '/con_0015_mni.nii,1'];
end

list_Button = [];
for c1 = 1:length(IDs)
    list_Button{c1,1} = [paths.analyses num2str(IDs(c1)) '_' num2str(days(c1,d)) '/con_0016_mni.nii,1'];
end
...
secondlvl_onesampleT(dir_spm,list_ffb_Neutrals_highDA_vs_lowDA) % run
fMRI_estimate([dir_spm '/SPM.mat'])
clear matlabbatch
spm_jobman('initcfg');      % initiate job manager
matlabbatch{1}.spm.stats.con.spmmat = cellstr([dir_spm '/SPM.mat']);
