function [CT] = simESRTM_1_0_0(varargin)
...
dt      = t(1) - 0;
cri     = dt*((cr(1)+0)/2);
CT      = (R1*cr(1) + k2*cri)/(1+(k2/(1+BP(1)))*(dt/2));
PI      = 0;
cti_last= dt*CT(1)/2;
for i=2:nr
    dt  = t(i)-t(i-1);
    ...
    if BP(i) == BP(i-1)
        CT = [CT; (PI + R1*cr(i) + k2*cri - (k2/(1+BP(i)))*(cti_last + dt*CT(i-1)/2))/(1+(k2/(1+BP(i)))*(dt/2))];
        cti_last = cti_last + dt*((CT(i)+CT(i-1))/2);
    else
        PI = PI - (k2/(1+BP(i-1)))*cti_last;
        cti_last = 0;
        CT = [CT; (PI + R1*cr(i) + k2*cri - (k2/(1+BP(i)))*(cti_last + dt*CT(i-1)/2))/(1+(k2/(1+BP(i)))*(dt/2))];
