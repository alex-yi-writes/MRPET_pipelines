...
template=/mnt/work/yyi/LIDO_OA_template.nii.gz
STUDYtemplte=/mnt/work/yyi/LIDO_STUDY_template.nii.gz

for K in  201 204 206 208 209 211 214 218 221 222 227 230 202 203 205 207 210 212 213 215 216 217 219 220 223 225 226 228 229
do
	...
	antsRegistrationSyN.sh -d 3 -n 2 -t s -f ${template} -m ${T1} -o ${folder_img}/NLreg_T1mean_to_OAtemplate_
	...
	mkdir ${folder_img}/OAtemplate
	mkdir ${folder_img}/STUDYtemplate


	# register images to the group template
	antsApplyTransforms -d 3 -v 0 -n BSpline[4] -t ${folder_img}/NLreg_T1mean_to_OAtemplate_1Warp.nii.gz -t "${folder_img}"/NLreg_T1mean_to_OAtemplate_0GenericAffine.mat -t "${folder_img}"/coreg_meanEPI_EM_to_T1mean_0GenericAffine.mat -i "${folder_img}"/${EPI} -r "${template}" -o "${folder_img}"/OAtemplate/NLreg_EPI_EM_on_OAtemplate.nii.gz
	for I in {01..08}
	do
	antsApplyTransforms -d 3 -v 0 -n Linear -t ${folder_img}/NLreg_T1mean_to_OAtemplate_1Warp.nii.gz -t "${folder_img}"/NLreg_T1mean_to_OAtemplate_0GenericAffine.mat -t "${folder_img}"/coreg_meanEPI_EM_to_T1mean_0GenericAffine.mat -i "${folder_img}"/con_00${I}_memory.nii -r "${template}" -o "${folder_img}"/OAtemplate/con_00${I}_memory_OAtemplate.nii.gz
	done

	for I in {01..12}
	do
	antsApplyTransforms -d 3 -v 0 -n Linear -t ${folder_img}/NLreg_T1mean_to_OAtemplate_1Warp.nii.gz -t "${folder_img}"/NLreg_T1mean_to_OAtemplate_0GenericAffine.mat -t "${folder_img}"/coreg_meanEPI_EM_to_T1mean_0GenericAffine.mat -i "${folder_img}"/con_00${I}_emoneutral.nii -r "${template}" -o "${folder_img}"/OAtemplate/con_00${I}_emoneutral_OAtemplate.nii.gz
	done

	for I in {01..07}
	do
	antsApplyTransforms -d 3 -v 0 -n Linear -t ${folder_img}/NLreg_T1mean_to_OAtemplate_1Warp.nii.gz -t "${folder_img}"/NLreg_T1mean_to_OAtemplate_0GenericAffine.mat -t "${folder_img}"/coreg_meanEPI_EM_to_T1mean_0GenericAffine.mat -i "${folder_img}"/con_00${I}_emoneutralnofix.nii -r "${template}" -o "${folder_img}"/OAtemplate/con_00${I}_emoneutralnofix_OAtemplate.nii.gz
	done
